...
class Trainer:
    ...
    def __init__(
        ...
        adam_optimizer={'beta1': 0.9, 'beta2': 0.999, 'epsilon': 1e-07},
        losses={
            'generator': 'mae',
    ...
    def train(self, epochs, steps_per_epoch, batch_size, monitored_metrics):
        ...
        if self.discriminator:
            ...
            valid = np.random.random([batch_size] + discr_out_shape) * 0.1 + 0.9
            fake = np.random.random([batch_size] + discr_out_shape) * 0.1
            validation_valid = np.random.random([len(validation_set['hr'])] + discr_out_shape) * 0.1 + 0.9
