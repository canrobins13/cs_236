...
class DataHandler:
    ...
    def _not_flat(self, patch, flatness):
        ...
        if flatness > 0 and max(np.std(patch, axis=0).mean(), np.std(patch, axis=1).mean()) < flatness:
            return False
        else:
            return True
    ...
    def _crop_imgs(self, imgs, batch_size, flatness):
        ...
        for slice_index, s in enumerate(slices['lr']):
            ...
            if (self._not_flat(candidate_crop, flatness) or n == 0) and np.count_nonzero(candidate_crop) / float(candidate_crop.size) > 0.9: 
